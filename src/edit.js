...
const Edit = ({ attributes, setAttributes, context }) => {
  const [editingTerm, setEditingTerm] = useState(0);
        ...
        >
          {attributeTerms?.map((term) => (
            <HStack justify="flex-start" key={term.id}>
              <ColorIndicator
                colorValue={termColors[term.id] ?? "#eeeeee"}
                onClick={() => setEditingTerm(term.id)}
              />
              <FlexItem title={term.name}>{term.name}</FlexItem>
              {editingTerm === term.id && (
                <Popover onFocusOutside={() => setEditingTerm(0)}>
                  <ColorPicker
                    color={termColors[editingTerm] ?? ""}
